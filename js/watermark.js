...
const Watermark = {
  image: null,        // 原始 ImageBitmap/HTMLImageElement
  canvas: null,       // 工作画布(原始分辨率)
  ctx: null,
  ...
  setImage(img) {
    ...
    document.getElementById('wmBadge').textContent = `${img.naturalWidth} × ${img.naturalHeight} · 原始分辨率`;
  ...
  layoutCanvas() {
    ...
    this.dispOffset = { x: rect.left, y: rect.top };
  },

  /** 屏幕坐标 -> 图像坐标 */
  toImageCoord(e) {
    const r = this.canvas.getBoundingClientRect();
    const x = (e.clientX - r.left) / this.dispScale;
    const y = (e.clientY - r.top) / this.dispScale;
    return { x: Math.max(0, Math.min(this.canvas.width, x)),
  ...
  getSelectedRect() {
    ...
    const w = Math.min(this.canvas.width - x, Math.round(r.w));
    const h = Math.min(this.canvas.height - y, Math.round(r.h));
    if (w <= 0 || h <= 0) return null;
    return { x, y, w, h };
  },

  /** ✨ 智能修复(Tela FMM + 细节填充),针对选区 */
  inpaintSelected() {
    ...
    const area = r.w * r.h;
    if (area > this.canvas.width * this.canvas.height * 0.5) {
  ...
  blurSelected() {
    ...
    if (!r) { Toast.show('请先框选水印区域'); return; }
    this.pushHistory();
    this.blurRect(r, Math.max(6, Math.round(Math.max(r.w, r.h) / 10)));
    this.rects = this.rects.filter((_, i) => i !== this.selIndex);
    this.selIndex = -1;
    this.redraw();
    Toast.show('已模糊选区');
  ...
  cropSelected() {
    ...
    const data = this.ctx.getImageData(r.x, r.y, r.w, r.h);
    const img = new Image();
    img.onload = () => {
      this.setImage(img);
      Toast.show('已裁剪出新图片');
  ...
  inpaintRect(r) {
    ...
    // 多尺度细节恢复:对修复区域与邻域做中值/均值平滑混合,减少色块感
    ...
    // 写回主图像:修复区域 + 平滑过渡(仅在选区边缘 6px 内混合,保持选区外 1:1)
    const edge = 6;
    for (let j = 0; j < bh; j++) {
      for (let i = 0; i < bw; i++) {
        ...
        const dEdge = Math.min(
          gx - x, x + w - 1 - gx, gy - y, y + h - 1 - gy
        );
        const t = dEdge < edge ? (edge - dEdge) / edge : 0; // 0=完全平滑, 1=完全不平滑
        const useSmooth = (buf[si] + smooth[si] + buf[si+1] + smooth[si+1] + buf[si+2] + smooth[si+2]) > 0;
        if (useSmooth) {
  ...
  blurRect(r, radius) {
    ...
    for (let j = 0; j < h; j++) {
      for (let i = 0; i < w; i++) {
        ...
        const p = (j * w + i) * 4;
        tmp[p] = r / n; tmp[p + 1] = g / n; tmp[p + 2] = b / n; tmp[p + 3] = buf[p + 3];
      }
    }
    // 垂直
    for (let j = 0; j < h; j++) {
